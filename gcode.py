...
def creategcode(currentConfig, version):
    print('[generator] Started creategcode')
    print('[generator] {}'.format(currentConfig))
...
G92 Z{zl:.3f}
G0 Z2 F600
M106 S{C}\n""".format(vs = version, T_h=currentConfig.temperature[0],
                      T_b=currentConfig.temperature[1],
                      C=int(currentConfig.def_cooling/100*255),
                      zl=currentConfig.def_layer,
                      zo=currentConfig.def_layer+currentConfig.z_offset,
                      F_t=currentConfig.def_speed_travel*60,
                      F_p=currentConfig.def_speed_print*60, X1=1, Y1=10,
                      Y2=currentConfig.bed_size[1]-10,
                      X2=1+currentConfig.def_line_width,
                      E1=ex.extrude(currentConfig.bed_size[1]-20),
                      E2 = ex.extrude(currentConfig.bed_size[1]-20),
                      ABL = ABL(currentConfig.use_ABL, currentConfig.ABL_type),
                      zeroadv = M900(0, currentConfig.firmware))
...
G0 X0 Y0 F{F_t}""".format(
        retr = "" if currentConfig.retract_at_layer_change else "\nG1 E-{R} F{RS}".format(
            R=currentConfig.retract[0],
            RS=currentConfig.retract[1]*60),
        F_t=currentConfig.def_speed_travel*60)
